...
/**
 * Base class that represents a row from the 'tipocaratula' table.
 *
 * 
 *
 ...
 */
abstract class BaseTipocaratula extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'TipocaratulaPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        TipocaratulaPeer
	 */
	 ...
	 * @return     Tipocaratula The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = TipocaratulaPeer::ID;
		}
	...
	public function setDescripcion($v)
	{
		...
		if ($this->descripcion !== $v) {
			$this->descripcion = $v;
			$this->modifiedColumns[] = TipocaratulaPeer::DESCRIPCION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = TipocaratulaPeer::NUM_COLUMNS - TipocaratulaPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Tipocaratula object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TipocaratulaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTipocaratula:delete:pre') as $callable)
			{
			...
			if ($ret) {
				TipocaratulaQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipocaratula:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTipocaratula:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipocaratula:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				TipocaratulaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TipocaratulaPeer::ID) ) {
					...
					$affectedRows = TipocaratulaPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TipocaratulaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = TipocaratulaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = TipocaratulaPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TipocaratulaPeer::ID)) $criteria->add(TipocaratulaPeer::ID, $this->id);
		if ($this->isColumnModified(TipocaratulaPeer::DESCRIPCION)) $criteria->add(TipocaratulaPeer::DESCRIPCION, $this->descripcion);
	...
	{
		$criteria = new Criteria(TipocaratulaPeer::DATABASE_NAME);
		$criteria->add(TipocaratulaPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Tipocaratula (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Tipocaratula Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     TipocaratulaPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new TipocaratulaPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this Tipocaratula is new, it will return
	 ...
	 * Otherwise if this Tipocaratula is new, it will return
	 * an empty collection; or if this Tipocaratula has previously
	 * been saved, it will retrieve related Impresions from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Tipocaratula.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseTipocaratula:' . $name))
		{
